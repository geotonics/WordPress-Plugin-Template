...
class WordPress_Plugin_Template_Template {
	...
	public function view_project_template( $template ) {
		...
		if ( file_exists( $file ) ) {
			...
			echo 'WordPress_Plugin_Template template file is missing:'.esc_html( $file );
		}
	...
	public function update_post_form() {
	    ...
	    wp_nonce_field( 'wordpress-plugin-template_save_post'.esc_html( $post_id ), 'wordpress-plugin-template_save_post'.esc_html( $post_id ).'_nonce' );
		echo '</div>
	        </form>';
		echo '<div>';
		die();
	...
	public function save_post() {
		$post_id = filter_input( INPUT_POST, 'post_id', FILTER_SANITIZE_NUMBER_INT );
		if ( isset( $post_id ) ) {
			$nonce_id = 'wordpress-plugin-template_save_post'.$post_id;
	...
	/**
	 * Main WordPress_Plugin_Template_Template Instance
	 *
	 * Ensures only one instance of WordPress_Plugin_Template_Template is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static.
	 *
	 * @param string $parent Name of parent class.
	 * @return Main WordPress_Plugin_Template_Template instance
	 */
